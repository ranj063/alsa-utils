	...
	snd_config_for_each(i, next, pipeline_cfg) {
		...
		if (!strcmp(id, "period")) {
			if (snd_config_get_integer(n, &sched_period)) {
				SNDERR("Invalid period for pipeline %s\n", id);
				return -EINVAL;
			}
		}
		...
		if (!strcmp(id, "rate")) {
			if (snd_config_get_integer(n, &rate)) {
				SNDERR("Invalid rate for pipeline %s\n", id);
				return -EINVAL;
			}
		}
	}
			     ...
			     snd_config_t *parent)
{
	...
	snd_config_for_each(i, next, cfg) {
		...
		snd_config_for_each(i2, next2, n) {
			...
			/*
			 * An object declared within a class defintion as follows:
